--============================================================
-- Dig & Clean - Full Hub (Obsidian UI)
-- v3
-- Ported from Ouroboros hub source, rebuilt on Obsidian
-- (deividcomsono/Obsidian). Tabs: Priority, Digging,
-- Cleaning, Storage, Shop + Settings.
--============================================================
...
local WIN_H = isMobile and 360 or 440

local Window = Library:CreateWindow({
	Title = "Qentury Hub",
	Footer = "Qentury Hub | Dig & Clean",
	Icon = LOGO_ID,
	NotifySide = "Right",
...
task.spawn(function()
	for _ = 1, 100 do
		...
		local obsidian = robloxGui and robloxGui:FindFirstChild("Obsidian")
		if not obsidian then
			continue
		end
		local icon
		for _, child in ipairs(obsidian:GetDescendants()) do
			if child:IsA("ImageLabel") and child.Image == "rbxassetid://" .. LOGO_ID then
				icon = child
				break
			end
		end
		...
		q.TextSize = icon.AbsoluteSize.X * 0.7
...
local PriorityStatusGroup = Tabs.Priority:AddLeftGroupbox("Status", "activity")
...
-- Obsidian splits groupboxes across two 50% columns regardless of left/right.
-- For single-column tabs (Priority, Digging, Cleaning): move the Status groupbox
-- into the main groupbox's column (Status on top) and stretch it, hide the other half.
local FULL_WIDTH_TABS = { "AFK Routine", "Auto Dig", "Auto Clean" }

local function consolidateTab(anchorTitle)
	...
	for _, child in ipairs(root:GetDescendants()) do
		if child:IsA("TextLabel") and child.Text == anchorTitle then
			anchor = child
			break
		end
	end
	...
	local tabCanvas = anchor
	while tabCanvas and not tabCanvas:IsA("CanvasGroup") do
		tabCanvas = tabCanvas.Parent
	end
	if not tabCanvas then
		return
	end
	local statusLabel
	for _, child in ipairs(tabCanvas:GetDescendants()) do
		if child:IsA("TextLabel") and child.Text == "Status" then
			statusLabel = child
			break
		end
	end
	local function boxOf(label)
		local holder = label.Parent
		return holder and holder.Parent
	end
	local anchorBox = boxOf(anchor)
	local statusBox = statusLabel and boxOf(statusLabel)
	local target = anchorBox and anchorBox.Parent
	if not target or not target:IsA("ScrollingFrame") then
		return
	end
	if statusBox and statusBox ~= anchorBox and statusBox.Parent ~= target then
		statusBox.LayoutOrder = 2
		anchorBox.LayoutOrder = 3
		statusBox.Parent = target
	end
	local container = target.Parent
	if not container then
		return
	end
	for _, side in ipairs(container:GetChildren()) do
		if side:IsA("ScrollingFrame") then
			if side == target then
				side.Visible = true
				side.Size = UDim2.new(1, -6, 1, 0)
				side.Position = UDim2.fromScale(0, 0)
			else
				side.Visible = false
				side.Size = UDim2.new(0, 0, 1, 0)
			end
		end
	end
end
...
task.spawn(function()
	for _ = 1, 6 do
		...
		for _, title in ipairs(FULL_WIDTH_TABS) do
			consolidateTab(title)
		end
	end
...
local function warnAgainstAutoDig(toggleName, label, description)
	Toggles[toggleName]:OnChanged(function(value)
		...
		local existing = activeDialogs[toggleName]
		if existing then
			pcall(function()
				existing:Dismiss()
			end)
		...
		activeDialogs[toggleName] = Window:AddDialog("Conflict" .. toggleName, {
