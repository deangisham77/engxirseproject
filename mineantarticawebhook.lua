...
local RadarRemotes
pcall(function()
	RadarRemotes = ReplicatedStorage:WaitForChild("RadarRemotes")
end)
local RadarShopState = RadarRemotes and RadarRemotes:WaitForChild("ShopState")

local RadarData
pcall(function()
	RadarData = require(ReplicatedStorage:WaitForChild("RadarData"))
end)
local RADAR_FALLBACK = {
	{ id = "CrystalRadar", displayName = "Crystal Radar" },
	{ id = "RarityRadar", displayName = "Rarity Radar" },
	{ id = "BoulderRadar", displayName = "Boulder Radar" },
	{ id = "RuneRadar", displayName = "Rune Radar" },
	{ id = "MeteorRadar", displayName = "Meteor Radar" },
	{ id = "ZenithRadar", displayName = "Zenith Radar" },
}
local function radarList()
	if type(RadarData) == "table" and type(RadarData.List) == "table" then
		return RadarData.List
	end
	return RADAR_FALLBACK
end
...
local lastRadarSig = nil
local lastRadarTotal = nil
local lastBoulder = nil
local lastRadarRefreshIn = nil
local radarSynced = false
...
local function radarTotal(stock)
	local n = 0
	for _, entry in ipairs(radarList()) do
		n += (stock[entry.id] or 0)
	end
...
local function radarLines(stock)
	local lines = {}
	for _, entry in ipairs(radarList()) do
		local c = stock[entry.id] or 0
		if c > 0 then
			table.insert(lines, string.format("• %s x%d", entry.displayName or entry.name or entry.id, c))
		end
	end
	if #lines == 0 then
		return "_Stok kosong semua._"
...
local function notifyRadar(stock, refreshIn)
	if not once("rr:" .. radarSig(stock), 900) then
		return
	end
	local boulder = stock["BoulderRadar"] or 0
	local isBoulder = boulder > 0 and (lastBoulder == 0 or lastBoulder == nil)
	local title = isBoulder and "🪨 BOULDER RADAR RESTOCK!" or "📡 Radar restock!"
	local desc = isBoulder and "Boulder Radar tersedia! Lihat boulder buried." or "Stok toko radar reset."
	local color = isBoulder and 2062263 or 5793266
	local extra = { { name = "Stok", value = radarLines(stock):sub(1, 900), inline = false } }
	if boulder > 0 then
		table.insert(extra, 1, { name = "Boulder", value = string.format("x%d — 12M, lihat boulder buried", boulder), inline = true })
	end
	if refreshIn and refreshIn > 0 then
		table.insert(extra, 1, { name = "Restock berikut", value = string.format("<t:%d:R>", os.time() + math.floor(refreshIn)), inline = true })
	end
	send(title, desc, color, extra, isBoulder)
end

local function handleRadarStock(stock, refreshIn)
	if type(stock) ~= "table" then
		return
	end
	local sig = radarSig(stock)
	local total = radarTotal(stock)
	local boulderNow = stock["BoulderRadar"] or 0
	if refreshIn then
		if lastRadarRefreshIn and lastRadarRefreshIn < 60 and refreshIn > 3000 and lastRadarSig and sig ~= lastRadarSig then
			notifyRadar(stock, refreshIn)
		elseif lastRadarTotal and total > lastRadarTotal + 1 then
			notifyRadar(stock, refreshIn)
		elseif lastBoulder == 0 and boulderNow > 0 then
			notifyRadar(stock, refreshIn)
		end
		lastRadarRefreshIn = refreshIn
	elseif lastRadarTotal == nil or sig ~= lastRadarSig then
		-- Poll GUI tanpa timer: update baseline diam saja.
	end
	lastRadarSig = sig
	lastRadarTotal = total
	lastBoulder = boulderNow
end

local function readRadarGuiStock()
	local ok, stock = pcall(function()
		local gui = LocalPlayer.PlayerGui:FindFirstChild("RadarShopGui")
		if not gui then
			return nil
		end
		local window = gui:FindFirstChild("Window")
...
-- Event: radar ShopState broadcast (stock + refreshIn)
if RadarShopState then
	STATE.connect(RadarShopState.OnClientEvent, function(p1)
		if typeof(p1) ~= "table" then
			return
		end
		if p1.stock then
			if not radarSynced then
				radarSynced = true
				lastRadarSig = radarSig(p1.stock)
				lastRadarTotal = radarTotal(p1.stock)
				lastBoulder = p1.stock["BoulderRadar"] or 0
				if p1.refreshIn then
					lastRadarRefreshIn = p1.refreshIn
				end
				refreshUI()
				return
			end
			handleRadarStock(p1.stock, p1.refreshIn)
		elseif p1.refreshIn then
			lastRadarRefreshIn = p1.refreshIn
		end
	end)
...
local uiBody, lblWeather, lblTimer, lblMeteor, lblMountain, lblBomb, lblRadar, lblLast, lblLog
...
function refreshUI()
	...
	lblRadar.Text = "Radar: "
		.. (lastRadarTotal ~= nil and (tostring(lastRadarTotal) .. " stok") or "?")
		.. (lastRadarRefreshIn and (" • restock " .. fmtCountdown(os.time() + math.floor(lastRadarRefreshIn))) or "")
...
do
	...
	frame.Size = UDim2.new(0, 260, 0, 271)
	...
	lblRadar = mkBody(85, 16, 13, Color3.fromRGB(140, 180, 255))
	lblLast = mkBody(102, 16, 12, Color3.fromRGB(170, 175, 190))
	lblLog = mkBody(120, 45, 11, Color3.fromRGB(130, 135, 150))
	lblLog.TextYAlignment = Enum.TextYAlignment.Top
	local btnTest = Instance.new("TextButton")
	btnTest.Position = UDim2.new(0, 10, 0, 185)
	...
	btnMin.Position = UDim2.new(0.62, 0, 0, 185)
	...
	STATE.connect(btnMin.MouseButton1Click, function()
		...
		lblRadar.Visible = open
		lblLast.Visible = open
		lblLog.Visible = open
		frame.Size = open and UDim2.new(0, 260, 0, 271) or UDim2.new(0, 260, 0, 62)
	end)
end
...
do
	...
	local radarStock = readRadarGuiStock()
	if radarStock then
		lastRadarSig = radarSig(radarStock)
		lastRadarTotal = radarTotal(radarStock)
		lastBoulder = radarStock["BoulderRadar"] or 0
		table.insert(extra, { name = "Stok radar (GUI)", value = radarLines(radarStock):sub(1, 900), inline = false })
	end
	send("✅ Monitor online", "Webhook jalan. Notifikasi: cuaca, meteor, bom restock, gunung, radar.", 4620980, extra)
	print("[webhook] monitor online, cuaca=" .. tostring(lastWeather) .. " meteor=" .. tostring(lastMActive) .. " gunung=" .. tostring(lastMountain) .. " radar=" .. tostring(lastRadarTotal))
	refreshUI()
end
...
while STATE.alive() do
	...
	if os.clock() - lastGuiPoll > 30 then
		...
		local rs = readRadarGuiStock()
		if rs then
			handleRadarStock(rs, nil)
		end
	end
